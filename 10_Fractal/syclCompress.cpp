...
void computeAffine(const vector<double> &D, const vector<double> &R, double &s, double &b) {
    ...
    s = (std::fabs(var) < 1e-6) ? 0 : cov / var;  // Use std::fabs to avoid ambiguity
    b = meanR - s * meanD;
}

void compress(const cv::Mat &img, int rangeSize, std::vector<Transform> &transforms, sycl::queue &q) {
...
void decompress(const vector<Transform> &transforms, int width, int height, int rangeSize, int iterations, Mat &outImg, sycl::queue &q) {
        ...
        for (const auto &T : transforms) {
            ...
            for (int i = 0; i < rangeSize; ++i) {
                for (int j = 0; j < rangeSize; ++j) {
                    double val = T.s * D[i * rangeSize + j] + T.b;
...
int main(int argc, char **argv) {
    if (argc < 4) {
        std::cout << "Usage: " << argv[0] << " <input.png/jpeg> <output.png> <iterations>\n";
        return 1;
    }
    ...
    sycl::queue q;  // Explicitly use sycl::queue
    compress(img, rangeSize, transforms, q);

    Mat outImg;
    decompress(transforms, img.cols, img.rows, rangeSize, iterations, outImg, q);
